    ...
    switch (format) {
      ...
      case 'bulletList':
        if (selectedText) {
          ...
          newText = value.substring(0, start) + bulletedText + value.substring(end);
        } else {
          newText = value.substring(0, start) + `- List item` + value.substring(end);
      ...
      case 'numberedList':
        if (selectedText) {
          const numberedText = selectedText.split('\n').map((line, index) => `${index + 1}. ${line}`).join('\n');
          newText = value.substring(0, start) + numberedText + value.substring(end);
        } else {
          newText = value.substring(0, start) + `1. List item` + value.substring(end);
      ...
      case 'quote':
        if (selectedText) {
          ...
          newText = value.substring(0, start) + quotedText + value.substring(end);
        } else {
          newText = value.substring(0, start) + `> Quote text` + value.substring(end);
          newSelectionStart = start + 2;
          newSelectionEnd = start + 12;
        }
        break;
    ...
    <div className="space-y-3 lg:space-y-4">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-gray-900 text-sm lg:text-base">Content</span>
        </div>
      ...
      {viewMode === 'edit' && (
        <div className=" rounded-lg">
          ...
          <div className={`${isToolbarExpanded ? 'block' : 'hidden'} lg:block p-3 lg:p-4 border-b bg-gray-50/50`}>
...
1. numbered list`}
          ...
          <div className="p-3 lg:p-4 border-t bg-gray-50/50">
            <div className="flex items-center gap-2 text-xs lg:text-sm text-gray-600">
              ...
              <span className="hidden sm:inline">Markdown: # H1, **bold**, *italic*, - list</span>
          ...
          <div className="p-4 lg:p-6">
            <div className="min-h-[400px] lg:min-h-[500px] p-4 lg:p-6 prose prose-sm lg:prose-lg max-w-none">
              {value ? (
                <ReactMarkdown
                  components={{
                    h1: ({ children }) => <h1 className="text-2xl lg:text-3xl font-bold mt-4 lg:mt-6 mb-3 lg:mb-4 border-b pb-2">{children}</h1>,
                    h2: ({ children }) => <h2 className="text-xl lg:text-2xl font-bold mt-3 lg:mt-5 mb-2 lg:mb-3">{children}</h2>,
                    h3: ({ children }) => <h3 className="text-lg lg:text-xl font-bold mt-2 lg:mt-4 mb-1 lg:mb-2">{children}</h3>,
                    p: ({ children }) => <p className="mb-3 lg:mb-4 leading-relaxed">{children}</p>,
                    ul: ({ children }) => <ul className="list-disc list-inside my-3 lg:my-4 space-y-1">{children}</ul>,
                    ol: ({ children }) => <ol className="list-decimal list-inside my-3 lg:my-4 space-y-1">{children}</ol>,
                    li: ({ children }) => <li className="ml-2 lg:ml-4">{children}</li>,
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-4 border-gray-300 pl-3 lg:pl-4 my-3 lg:my-4 italic text-gray-700">
                        {children}
                      </blockquote>
                    ),
                    code: ({ children, className }) => {
                      ...
                      if (isInline) {
                        return <code className="bg-gray-100 px-1 rounded text-xs lg:text-sm">{children}</code>;
                      }
                      return (
